...
2. 主程序在调用函数时，函数调用必须是在定义之后，不然解释器无法解释，但是函数与函数间的调用无所谓顺序
3. 函数的前后都需要有两行空行
4. 函数的注释是写在方法内部的
...
print_info(*(1, 2, 3), **{"name": "小明",
                          "age": 18})
"""
__name__是某模块的名字变量，当前执行的程序是本模块时，__name__ 值就是__main__ if条件成立, 
若是被当模块被导入__name__为当前模块(文件)名称，if条件不成立，
这样就完美解决了模块非方法函数被运行的问题，可以让模块既可以独立运行，也可以被其他模块导入使用。
"""
