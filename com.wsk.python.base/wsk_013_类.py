...
11. 当属性的申明是直接在内部时，此属性就是类属性，类似于java的静态变量，可直接通过类名称去调用。 对象.xx方式使用属性其的查找顺序是先从对象属性查找，查不到再从类属性查找。 访问类属性不建议使用
...
13 py中如果一个方法需要使用懂对象属性，建议创建为对象方法，如果只访问类属性，建议创建为类方法，如果什么属性都不访问，建议创建为静态方法(@staticmethod修饰)
"""
print("demo".__doc__)
print(dir("demo"))
