...
2. 变量的类型主要分为数字型和非数字型
    ...
    非数字类型：字符串、列表、元组、字典
3. 标识符(变量、函数名称、模块名)由字母、下划线、数字组成、但是数字不能开头，不能与关键字重名
5. del 是将变量从内存中删除
6. 变量存储的是对象地址的引用
7. py无法直接在函数中给全局变量进行修改的，如果进行新的赋值/写操作（=、+=、.update() 等），只是在函数中创建了同名的局部变量，该变量值只在此方法中有效，如果想改全局变量，需要用
...
10. 单引号，双引号，三引号(三双或者三单)都可以用于描述字符串，前两者区别在于字符串中有单或者双引号，如果不想\转义，则用对应双、单括在最外面，而后者可以换行写代码不需要\n 转义
11. 字符串前加u表示为unicode编码，解决中文乱序问题，r表示一个普通生字符串,里面的转义字符无效，b表示表示这是一个 bytes 对象
"""
# 申明无需指定类型
...
""" 列表
1. 列表可以存储不同类型的数据，但是实际使用中都是存储一种数据
2. 列表中数据可以增删改
"""
...
1. 在py中除列表外，最常用的集合
2. 列表是有序的，字典数据是无序的
3. 字典由key value组成，其中key只能是字符串或者数字等不可变数据类型，值为任意类型
"""
...
print((4, 5, 6, 7)[::-1])
print("hello {}, 我来自 {}".format("wsk", "火星"))  # 字符串拼接
